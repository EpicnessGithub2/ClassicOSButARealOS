...
classicOS code. Two versions are built, "inflib_host" (for use by build tools)
and "inflib" (for use by classicOS code). Both depend on the same core source,
with a wrapper for the appropriate interface.
Most of the differences between the host and the classicOS environment are
abstracted away in builddep.h. Of particular note is that the host version
uses Ansi characters while the classicOS version uses Unicode. So, the core
...
when building the classicOS version (inflib.xml).
The wrappers have "host" or "ros" in their filename. The library interface is
"infhost.h" for the host version, "infros.h" for the classicOS version.
