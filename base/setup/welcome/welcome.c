...
LoadLocalizedResourcesFromINI(LCID Locale, LPTSTR lpResPath)
{
    ...
    GetPrivateProfileString(TEXT("Defaults"), TEXT("AppTitle"), TEXT("classicOS - Welcome") /* default */,
...
LoadConfiguration(VOID)
{
    ...
    if (!LoadString(hInstance, IDS_APPTITLE, szAppTitle, ARRAYSIZE(szAppTitle)))
        StringCchCopy(szAppTitle, ARRAYSIZE(szAppTitle), TEXT("classicOS - Welcome"));
...
RunAction(INT nTopic)
{
    ...
    if (!pTopics[nTopic]->bIsCommand)
    {
        ...
        {
            MessageBox(hWndMain, Command + 5, TEXT("classicOS"), MB_OK | MB_TASKMODAL);
            return TRUE;
        }
    }
    else
...
OnPaint(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
    ...
    StringCchCopy(szVersion, ARRAYSIZE(szVersion),
                  TEXT("classicOS ") TEXT(KERNEL_VERSION_STR));
