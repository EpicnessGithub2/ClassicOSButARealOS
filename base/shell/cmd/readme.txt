...
The classicOS command line interpreter CMD is derived from FreeCOM, the
...
Compiling
~~~~~~~~~
ROS cmd used to depend on __classicOS__ to provide two different ways to build cmd.  There is still code left in it for this but...  The __classicOS__ = 0 has not been develped, maintained.  And therefore it does not even compile anymore.  __classicOS__ = 1 works fine on both windows(nt). and someday i plan to remove all the __classicOS__ = 0.
